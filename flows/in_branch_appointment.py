"""In-Branch Appointment → address → date → time → brand → Skip note → Send now."""

from __future__ import annotations

import re

TEMPLATE_NAME = "In-Branch Appointment"
NEEDS_BANKS = True
...
def run(message, send_message, bank: str, h: dict) -> None:
    ...
    send_text(
        ...
        send_message,
        cfg.get(
            "in_branch_appointment_address",
            cfg.get("in_branch_address", "420 Montgomery St, San Francisco, CA 94104"),
        ),
    )
    send_text(
        message,
        send_message,
        cfg.get("in_branch_appointment_date", "September 12, 2026"),
    )
    send_text(
        message,
        send_message,
        cfg.get("in_branch_appointment_time", "10:30 AM"),
    )

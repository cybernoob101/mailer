"""Rep Verification → specialist → phone → institution → brand → Skip note → Send now."""

from __future__ import annotations

import re

TEMPLATE_NAME = "Rep Verification"
NEEDS_BANKS = True
...
def run(message, send_message, bank: str, h: dict) -> None:
    ...
    send_text(
        message,
        send_message,
        cfg.get("rep_verification_name", "Jordan Blake"),
    )
    send_text(
        message,
        send_message,
        cfg.get("rep_verification_phone", "1234567890"),
    )
